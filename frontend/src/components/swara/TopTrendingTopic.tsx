...
export default function TopTrendingTopic({ batch }: Props) {
  const [rootCauseData, setRootCauseData] = useState<RootCauseStats[]>([]);

  useEffect(() => {
    axiosInstance
      .get<{ data: RootCauseStats[] }>("/stats/root-cause", { params: { program_batch: batch } })
      .then((response) => setRootCauseData(response.data.data))
      .catch((error) => console.error("Failed to get trending topic:", error));
  }, [batch]);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6 pb-5 sm:pb-6">
      <h3 className="text-lg font-semibold text-brand-500 dark:text-white/90 mb-6 mt-1">
          ...
          return (
            <div key={item.root_cause}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm text-gray-700 dark:text-gray-300">{item.root_cause}</span>
