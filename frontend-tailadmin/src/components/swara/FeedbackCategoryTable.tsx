...
export default function FeedbackCategoryTable({ categoryOptions }: Props) {
  const tableRef = useRef<any>(null);
  const wrapperRef = useRef<HTMLDivElement>(null);
  const [category, setCategory] = useState("");
  const [period, setPeriod] = useState("");

  // Ref buat nyimpen nilai filter TERBARU, dibaca langsung oleh ajax()
  // (bukan lewat closure state yang bisa stale)
      ...
      <div ref={wrapperRef} className="swara-datatable-wrapper">
        <DataTable
          ...
          ajax={(dtParams: any, callback: (res: any) => void) => {
            // Baca dari REF, bukan dari state -- selalu nilai terbaru
